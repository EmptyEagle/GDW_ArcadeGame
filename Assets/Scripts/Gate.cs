...
public class Gate : MonoBehaviour
{
    ...
    // Delay the update function so it doesn't check for button states every frame
    private float delayBeforeNextCheck = 1f;
    private bool canCheckButtonStates;
    
    // Start is called once before the first execution of Update after the MonoBehaviour is created
    void Start()
    {
        startPosY = transform.position.y;
        canCheckButtonStates = true;
    }

    // Update is called once per frame
    void Update()
    {
        if (canCheckButtonStates)
        {
